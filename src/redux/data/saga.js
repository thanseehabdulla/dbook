...
export function* watcherSaga() {
    ...
    yield takeLatest(DATA_ACTIONS.DELETE_PURCHASE, workerdeletePurchaseSaga);
    yield takeLatest(DATA_ACTIONS.DELETE_SALES, workerdeleteSalesSaga);
...
// worker saga: makes the api call when watcher saga sees the action
function* workerdeletePurchaseSaga(payload) {
    try {
        let body = {
            url: API.PURCHASE_API+ "/" +payload.purchaseid
        }
...
// worker saga: makes the api call when watcher saga sees the action
function* workerdeleteSalesSaga(payload) {
    try {
        let body = {
            url: API.SALES_API+ "/" +payload.salesid
        }
        const response = yield call(REQUEST.deleteData, body);
        const data = response.data;
